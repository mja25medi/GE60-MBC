...
<form name="orgImgInfoForm" id="orgImgInfoForm" onsubmit="return false" method="POST">
	...
	<table summary="" class="table table-bordered">
		...
		<tr>
			...
			<td>
				<c:if test="${itemDTO.imgTypeCd eq 'MAINIMG'}">
					<c:choose>
						<c:when test="${colorTplCd eq '001'}"><p><spring:message code="org.message.imginfo.size.main"  arguments="1170|400"  argumentSeparator="|"/></p></c:when>
				...
				<c:if test="${itemDTO.imgTypeCd eq 'SUBIMG'}">
					<c:choose>
				...
				<c:if test="${itemDTO.imgTypeCd eq 'LECIMG'}">
					<c:choose>
		...
		<tr>
			...
			<td >
				<input type="text" name="connTrgt" id="connTrgt" maxlength="50" title="<spring:message code="org.title.imginfo.linktarget"/>" class="form-control input-sm" value="${vo.connTrgt}"/>
			</td>
		</tr>
		</c:if>
	</table>
...
<script type="text/javascript">
	...
	function process(cmd) {
		...
		$('#orgImgInfoForm').attr("action",cmd);
	 ...
	 */
	function addImg() {
		process("/adm/org/imginfo/add");	// cmd
	}
	 ...
	 */
	function editImg() {
		process("/adm/org/imginfo/edit"); // cmd
	}
	...
	function delImg() {
		if(confirm('<spring:message code="org.message.imginfo.confirm.delete"/>')) {
			process("/adm/org/imginfo/remove"); // cmd
		} else {
			return;
		}
	}
