...
<%
	...
	List<SysMenuVO> sysMenuList = menuService.getAdmMenuList(authGrpCd);
	request.setAttribute("sysMenuList",sysMenuList);
%>
	<header class="main-header">
		<!-- Logo -->
		<a href="/index.jsp" class="logo" target="_top">
			...
			<span class="logo-lg">
				<c:choose>
					...
					<img src="<c:url value="/app/file/view2/${ADM_LOGOSN}"/>" alt="${USER_ORGNM}"/>
					</c:when>
					<c:otherwise>
					${USER_ORGNM}
					</c:otherwise>
						...
						<ul class="dropdown-menu">
							...
							<li class="user-footer">
								...
								<div class="pull-right">
									<a href="/adm/main/logout" class="btn btn-default btn-flat">Logout</a>
								</div>
							</li>
						</ul>
					</li>					
			...
			<ul class="sidebar-menu" id="sidebar-menu">
				...
				<c:forEach items="${sysMenuList}" var="item1">
					...
					<c:if test="${empty item1.subList}">
						<c:url var="menuLink1" value="/adm/main/goMenuPage?mcd=${item1.menuCd}"/>
					</c:if>				
					...
					<ul class="treeview-menu">
						<c:forEach items="${item1.subList}" var="item2">
							...
							<c:if test="${empty item2.subList}">
								<c:url var="menuLink2" value="/adm/main/goMenuPage?mcd=${item2.menuCd}"/>
							</c:if>	
						<li <c:if test="${item2.menuCd eq chkedmenu2}">class="active"</c:if>>
							...
							<ul class="treeview-menu">
								<c:forEach items="${item2.subList}" var="item3">
									...
									<c:url var="menuLink3" value="/adm/main/goMenuPage?mcd=${item3.menuCd}"/>
								<li>
									<a href="${menuLink3}">
