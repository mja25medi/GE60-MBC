...
<c:set var="transVO" value="${vo.logMsgTransLogVO}"/>
    ...
    <table class="table table-striped table-bordered">
    	...
    	<tr>
    		...
    		<td class="top">
    			<input type="text" style="width:100" dispName="<spring:message code="log.title.msg.sender.number"/>"  maxlength="14" isNull="N" lenCheck="14" name="sendAddr" id="txtSendAddr" class="form-control input-sm" value="${vo.sendAddr}"/>
    		</td>
    	</tr>
    	<tr>
			...
			<td colspan="2">
				<div id="sendControl">
					<div class="input-group" style="float:left;width:140px;">
						<input type="text"  maxlength="10" dispName="<spring:message code="common.title.day"/>" isNull="Y" name="rsrvDate" id="rsrvDate" class="form-control input-sm inputDate"/>
					...
					<div style="float:left">
						<input type="text" style="width:40px" dispName="<spring:message code="common.title.hour"/>" maxlength="2" isNull="Y" lenCheck="2"  name="rsrvHour" id="rsrvHour" onkeyup="checkHour()" class="form-control input-sm" placeholder="<spring:message code="common.title.hour"/>"/>
					</div>
					<div style="float:left">
						<input type="text" style="width:40px" dispName="<spring:message code="common.title.min"/>"  maxlength="2" isNull="Y" lenCheck="2" name="rsrvMin" id="rsrvMin" onkeyup="checkMinute()" class="form-control input-sm" placeholder="<spring:message code="common.title.min"/>"/>
					</div>
				</div>
    		</td>
    	</tr>
...
<script type="text/javascript">
			...
			if($M.Check.Event.isClickEnter(event)) {
				...
				if( $(':input:radio[name=sendType]:checked').val() == 'R' ) {
					...
					if(checkHour() == false || $('#rsrvHour').val() == ""
						|| checkMinute() == false || $('#rsrvMin').val() == "") {
						alert("전송시간을 입력하세요.");
						$('#rsrvHour').focus();
						return;
					}
					
					if($('#rsrvHour').val().length == 1){
						$('#rsrvHour').val('0'+$('#rsrvHour').val());
					}
					
					if($('#rsrvMin').val().length == 1){
						$('#rsrvMin').val('0'+$('#rsrvMin').val());
					}
					
				}else{//예약이 아니면 예약 날짜 초기화
					$('#rsrvDate').val('');
					$('#rsrvHour').val('');
					$('#rsrvMin').val('');
				}
	...
	function checkHour(){
		...
		if((value > 23) || (value < 0)){
			//alert("입력 가능한 시간(시)이 아닙니다. (00 ~ 23 사이 값 입력)");
			alert('<spring:message code="log.message.sms.rsrv.alert.validate.hour"/>');
			$('#rsrvHour').val('').focus();
			return false;
		}
		return true;
	...
	function checkMinute(){
		...
		if((value > 59) || (value < 0)){
			//alert("입력 가능한 시간(분)이 아닙니다. (00 ~ 59 사이 값 입력)");
