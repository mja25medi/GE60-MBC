...
$.fn.pagingHtml = function(pageInfo, func_name, img_path) {
	...
	var intStartPage = (Math.ceil(pageInfo.currentPage/pageInfo.pageScale)-1) * pageInfo.pageScale + 1;
	...
	if (intStartPage > 1) {
		page +=	"<a href=\"javascript:"
				+ func_name
				+ "("
				+ (intStartPage - pageInfo.pageScale)
	...
	for (var i = intStartPage; i < intStartPage + pageInfo.pageScale; i++) {
		iTemp = i;
	...
	if (pageInfo.currentPage < pageInfo.totalPageCount) {
		page +=	"<a href=\"javascript:"
				+ func_name
				+ "("
				+ (pageInfo.currentPage + 1)
