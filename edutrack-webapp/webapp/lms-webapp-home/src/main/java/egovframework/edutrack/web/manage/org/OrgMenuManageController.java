...
@RequestMapping(value="/mng/org/menu")
public class OrgMenuManageController extends GenericController {
	 ...
	 * @param commandMap
	 * @param model
	 * @return  "/mng/org/menu/list_menu.jsp"
	 * @throws Exception
	 */
	 ...
	 * @param commandMap
	 * @param model
	 * @return  "/mng/org/menu/write_menu.jsp"
	 * @throws Exception
	 */
	@RequestMapping(value="/addMenuPop")
	public String addMenuPop(OrgMenuVO vo, Map commandMap, ModelMap model, 
			HttpServletRequest request) throws Exception {
		commonVOProcessing(vo, request);
		String orgCd = UserBroker.getUserOrgCd(request);
		vo.setOrgCd(orgCd);
		...
		return "mng/org/menu/write_menu_pop";
	}
	
	/**
     * @Method Name : addFormSubMenu
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/addSubMenuPop")
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/editMenuPop")
	public String editFormMenu(OrgMenuVO vo, Map commandMap, ModelMap model, 
			HttpServletRequest request) throws Exception {
		commonVOProcessing(vo, request);
		String orgCd = UserBroker.getUserOrgCd(request);
		vo.setOrgCd(orgCd);
		...
		if(url != null && !"".equals(url) && url.contains("&ctgrs=") && url.contains("&types=")){
			
			String[] typesArr;
			String[] ctgrsArr;
			
			typesArr = url.split("&types=");
			if(typesArr.length > 1){
				vo.setKnowTypes(typesArr[1]);
			}
			
			ctgrsArr = typesArr[0].split("&ctgrs=");
			if(ctgrsArr.length > 1){
				vo.setKnowCtgrs(ctgrsArr[1]);
			}
		}
		/*END 지식메뉴 분류 및 자료유형 처리*/
	...
	public String sortMenu(OrgMenuVO vo, Map commandMap, ModelMap model, 
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		commonVOProcessing(vo, request);
	 ...
	 * @param commandMap
	 * @param model
	 * @return  "/mng/org/menu/list_bbs.jsp"
	 * @throws Exception
	 */
	 ...
	 * @param commandMap
	 * @param model
	 * @return  "/mng/org/menu/list_page.jsp"
	 * @throws Exception
	 */
	...
	public String listPage(OrgMenuVO vo, Map commandMap, ModelMap model, 
		...
		return "mng/org/menu/list_page";
	}	
	
	/**
	 * @Method Name : addFormDirect
	 * @Method 설명 : 직접입력
	 * @param OrgMenuVO 
	 * @param commandMap
	 * @param model
	 * @return  "/mng/org/menu/write_direct.jsp"
	 * @throws Exception
	 */
	@RequestMapping(value="/addFormDirect")
	public String addDirect(OrgMenuVO vo, Map commandMap, ModelMap model, 
			HttpServletRequest request) throws Exception {
		commonVOProcessing(vo, request);
		String orgCd = UserBroker.getUserOrgCd(request);
		vo.setOrgCd(orgCd);
		
		return "mng/org/menu/write_direct";
	}	
	
	
	/**
     * @Method Name : makeUrl
     * @Method 설명 : 지식 url 을 생성한다
	 * @param commandMap
	 * @param model
	 * @return  json
	 * @throws Exception
	 */
	@RequestMapping(value="/makeUrl")
	public String makeUrl(OrgAuthGrpMenuVO vo, Map commandMap, ModelMap model, 
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		commonVOProcessing(vo, request);
		
		String ctgrs = (String)commandMap.get("ctgrs");
		String types = (String)commandMap.get("types");
		
		if(!"".equals(ctgrs)){
			ctgrs = KISASeed.seedEncryption(ctgrs);
		}
		if(!"".equals(types)){
			types = KISASeed.seedEncryption(types);
		}
		
 		commandMap.put("ctgrs", ctgrs);
 		commandMap.put("types", types);
		
		return JsonUtil.responseJson(response, commandMap);
	}
