...
<%@ include file="/WEB-INF/jsp/mng/common/page_init.jsp" %>
			<table summary='회원로그인 api' class="table table-bordered">
				...
				<tbody>
					<c:choose>
						<c:when test="${empty scoreApiList}">
							<tr>
						...
						<c:otherwise>
							...
							<c:forEach items="${scoreApiList}" var="item" varStatus="status">
								<c:choose>
							...
							<tr>
								...
								<td class="text-center">
									<c:choose>
										<c:when test="${syncWaitCnt > 0}">
											<a href="javascript:viewScoreApiSyncPop('SCORE','W');">${syncWaitCnt}</a>
										</c:when>
										<c:otherwise>
											${syncWaitCnt}
										</c:otherwise>
								...
								<td class="text-center">
									<button type="button" class="btn btn-info btn-sm" onclick="javascript:callScoreHrdApi();">발송</button>
									<button type="button" class="btn btn-info btn-sm" onclick="javascript:viewApiLogPop('SCORE');">이력</button>
								</td>
							</tr>
						</c:otherwise>
					</c:choose>
		...
		<script type="text/javascript">
			...
			function viewScoreApiSyncPop(syncStatus,syncStatus){
				//var searchFrom = $("#userLoginSearchFrom").val();
				//var searchTo = $("#userLoginSearchTo").val();
				var crsCd = $("#scoreCrsCd").val();
				if(crsCd != ''){
					...
					var url = generateUrl("/mng/etc/HrdApi/viewScoreApiSyncPop",{"syncStatus":syncStatus,"crsCd":crsCd});
					var addContent  = "<iframe id='failApiFrame' name='failApiFrame' width='100%' height='100%' "
						+ "frameborder='0' scrolling='auto' src='"+url+"'/>";
					var title = "";
					modalBox.clear();
					modalBox.addContents(addContent);
					modalBox.resize(1200, 600);
