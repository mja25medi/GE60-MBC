...
<c:set var="itemList" value="${eventList}"/>
	<form name="eventForm" id="eventForm">
    <input type="hidden" name="eventSns" value="${vo.eventSns }" id="eventSns"/>
    <input type="hidden" name="eventNm" styleId="eventNm" value="1111"/>
	<table title="<spring:message code="etc.title.event.sort"/>" style="width:100%">
		<tr>
			<td style="width:65%;padding:5px;" valign="top">
				<select name="eventSn" id="eventSn" multiple style="width:100%;height:250px" class="form-control input-sm">
				<c:if test="${not empty itemList}">
					<c:forEach var="item" items="${itemList}">
					<option value='${item.eventSn}'>${item.eventNm}</option>
					</c:forEach>
				</c:if>
				</select>
			</td>
...
<script type="text/javascript">
	...
	function sortCode() {
		...
		$("#eventSns").val("");
		var selectBox = document.getElementById("eventSn");
