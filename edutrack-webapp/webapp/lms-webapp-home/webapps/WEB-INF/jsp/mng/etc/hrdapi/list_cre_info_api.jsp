...
<%@ include file="/WEB-INF/jsp/mng/common/page_init.jsp" %>
			<table summary='수업정보 api' class="table table-bordered">
				<colgroup>
				...
				<tbody>
					<c:choose>
						<c:when test="${empty creInfoApiList}">
							<c:choose>
								<c:when test="${syncTotalCnt > 0}">
									<tr>
										<td class="text-center">수업 정보</td>
										<td class="text-center">${syncTotalCnt}</td>
										<td class="text-center">0</td>
										<td class="text-center">0</td>
										<td class="text-center">0</td>
										<td class="text-center">
											<button type="button" class="btn btn-info btn-sm" onclick="javascript:callCreInfoHrdApi();">발송</button>
										</td>
									</tr>
								</c:when>
								<c:otherwise>
						...
						<c:otherwise>
							...
							<c:forEach items="${creInfoApiList}" var="item" varStatus="status">
								<c:choose>
							...
							<tr>
								...
								<td class="text-center">
									<c:choose>
										<c:when test="${syncFailCnt > 0}">
											<a href="javascript:viewCreApiSyncPop('CRE_INFO','F');">${syncFailCnt}</a>
										</c:when>
										<c:otherwise>
											${syncFailCnt}
										</c:otherwise>
								...
								<td class="text-center">
									<c:choose>
										<c:when test="${syncWaitCnt > 0}">
											<a href="javascript:viewCreApiSyncPop('CRE_INFO','W');">${syncWaitCnt}</a>
										</c:when>
										<c:otherwise>
											${syncWaitCnt}
										</c:otherwise>
								...
								<td class="text-center">
									<button type="button" class="btn btn-info btn-sm" onclick="javascript:callCreInfoHrdApi();">발송</button>
									<button type="button" class="btn btn-info btn-sm" onclick="javascript:viewApiLogPop('CRE_INFO');">이력</button>
								</td>
							</tr>
						</c:otherwise>
					</c:choose>
		...
		<script type="text/javascript">
			...
			//발송
			function callCreInfoHrdApi() {
				var crsCd = $("#creInfoCrsCd").val();
				
				if(crsCd != ''){
					$.ajax({
						url:cUrl("/mng/etc/HrdApi/callCreInfoHrdApi"),
						type:"POST", 
						data:{
							"crsCd" : crsCd,
							"syncGubunCd": "CRE_INFO",
							"syncStatus":"W"
						},
						success:function(data){
							if(data.result > 0) {
								alert("전송에 성공하였습니다.")		
								parent.listCreApiInfo();
							} else {
			...
			//실패리스트
			function viewCreApiSyncPop(syncGubunCd,syncStatus){
				var crsCd = $("#creInfoCrsCd").val();
				if(crsCd != ''){
					var url = generateUrl("/mng/etc/HrdApi/viewCreApiSyncPop",{"syncGubunCd":syncGubunCd,"crsCd":crsCd,"syncStatus":syncStatus});
