...
<%@ include file="/WEB-INF/jsp/mng/common/page_init.jsp" %>
								...
								<td class="text-center">
									<c:choose>
										<c:when test="${syncFailCnt > 0}">
											<a href="javascript:viewUserInfoApiSyncPop('USER_INFO','F');">${syncFailCnt}</a>
										</c:when>
										<c:otherwise>
											${syncFailCnt}
										</c:otherwise>
								...
								<td class="text-center">
									<c:choose>
										<c:when test="${syncWaitCnt > 0}">
											<a href="javascript:viewUserInfoApiSyncPop('USER_INFO','W');">${syncWaitCnt}</a>
										</c:when>
										<c:otherwise>
											${syncWaitCnt}
										</c:otherwise>
		...
		<script type="text/javascript">
			...
			function viewUserInfoApiSyncPop(syncGubunCd,syncStatus){
				var url = generateUrl("/mng/etc/HrdApi/viewUserInfoApiSyncPop",{"syncGubunCd":syncGubunCd,"syncStatus":syncStatus});
