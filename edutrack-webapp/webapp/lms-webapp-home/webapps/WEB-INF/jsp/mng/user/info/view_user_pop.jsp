		...
		<table summary="<spring:message code="user.title.userinfo.user.info"/>" id="userDetailInfoTable" class="table table-bordered wordbreak">
			...
			<c:choose>
				<c:when test="${not empty vo.snsDiv }">
					<tr>
						<th scope="row"><spring:message code="org.title.orginfo.orgname"/></th>
						<td><spring:message code="user.title.userinfo.sns.${vo.snsDiv }"/></td>
					</tr>
					<tr>
						<th scope="row"><spring:message code="user.title.userinfo.sns.snskey"/>(<spring:message code="user.title.userinfo.userno"/>)</th>
						<td >${vo.snsKey}(${vo.userNo})</td>
					</tr>
				</c:when>
				<c:otherwise>
					<tr>
					...
					<tr>
						<th scope="row"><spring:message code="user.title.userinfo.userno"/></th>
						<td >${vo.userNo}</td>
					</tr>
				</c:otherwise>
			</c:choose>
...
<c:if test="${cfgItem.fieldNm eq 'PHOTO'  && cfgItem.useYn eq 'Y' }">
			...
			<tr class="option_view">
				...
				<td>
					<c:if test="${not empty vo.photoFileSn && vo.photoFileSn > 0}">
...
<c:if test="${cfgItem.fieldNm eq 'BIRTH' && cfgItem.useYn eq 'Y' }">
			<tr class="option_view">
				...
				<td><meditag:dateformat type="1" delimeter="-" property="${vo.birth}"/></td>
			</tr>
</c:if>
...
<c:if test="${cfgItem.fieldNm eq 'USERDIV' && cfgItem.useYn eq 'Y' }">
			<tr class="option_view">
				<th scope="row"><spring:message code="common.title.type2"/></th>
				<td>${vo.userDivNm }</td>
			</tr>
</c:if>
...
<c:if test="${cfgItem.fieldNm eq 'JOB' && cfgItem.useYn eq 'Y' }">
			<tr class="option_view">
				<th scope="row"><spring:message code="user.title.userinfo.manage.job"/></th>
				<td>${vo.jobNm }</td>
			</tr>
</c:if>
			...
			<tr class="option_view">
				<th scope="row"><spring:message code="user.title.userinfo.manage.knowctgr"/></th>
				<td colspan="3">
					<c:set var="orgKnowCtgrStr" value=""/>
					<c:forEach items="${orgKnowCtgrCdList}" var="item" varStatus="status">
						<c:if test="${fn:contains(vo.usrKnowCtgrCds, item.knowCtgrCd) }">
							<c:if test="${not empty orgKnowCtgrStr}">
								<c:set var="orgKnowCtgrStr" value="${orgKnowCtgrStr},"/>
							</c:if>
							<c:set var="orgKnowCtgrStr" value="${orgKnowCtgrStr}${item.knowCtgrNm}"/>
						</c:if>
					</c:forEach>
					${orgKnowCtgrStr }
				</td>
			...
			<tr class="option_view">
				<th scope="row"><spring:message code="user.title.userinfo.manage.knowkeyword"/></th>
				<td colspan="3">
					${vo.usrKnowKeywords }
				</td>
			</tr>
...
<script type="text/javascript">
	...
	function userInfoEdit() {
		...
		document.location.href = cUrl("/mng/user/userInfo/editUserPop")+"?userNo=${vo.userNo}&orgCd=${vo.orgCd}";
	}
</script>
