...
<mhtml:mng_head titleName="${MENUNAME}">
	...
	<form id="tchWritingsForm" name="tchWritingsForm" onsubmit="return false" >
		...
		<input type="hidden" name="lecWritingsSn" value="${vo.lecWritingsSn }" />
		...
		<table summary="<spring:message code="teacher.title.teacherinfo.teacher.manage"/>" class="table table-bordered wordbreak">
			...
			<tr >
			<th scope="row"><label for="cts"><label for="cts"><spring:message code="teacher.title.teacherinfo.writing.cts"/></label></th>
			<td colspan="3">
		...
		<div class="text-right">
			...
			<a href="javascript:delWrite()" class="btn btn-primary btn-sm"><spring:message code="button.delete"/> </a>
			</c:if>

			<a href="/mng/tch/info/editPop?userNo=${tchWritingsVO.userNo}" class="btn btn-default btn-sm"><spring:message code="button.close"/> </a>
		</div>
...
<script type="text/javascript">
	...
	function process(cmd) {
		$('#tchWritingsForm').attr("action", "/mng/tch/writing/" + cmd);
	...
	function processCallback(resultDTO) {
		...
		if(resultDTO.result >= 0) {
			// 정상 처리
			document.location = "/mng/tch/info/editPop?userNo=${tchWritingsVO.userNo}";
		} else {
			// 비정상 처리
		}
	}
