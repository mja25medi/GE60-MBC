...
<mhtml:mng_head titleName="${MENUNAME}">
	...
	<form id="tchSchsForm" name="tchSchsForm" onsubmit="return false">
		...
		<table summary="<spring:message code="teacher.title.teacherinfo.teacher.manage"/>" class="table table-bordered wordbreak">
			<colgroup>
				<col width="18%"/>
				<col width="32%"/>
				<col width="18%"/>
					...
					<div class="input-group">
						<select name="entrDt_year" id="entrDt_year" class="form-control input-sm" style="width:100px;float:left;">
							<c:forEach var="i" begin="0" end="${year-1900}">
								<c:choose>
									<c:when test="${fn:substring(tchSchsVO.entrDt,0,4) eq (year-i)}"><option selected value="${year-i}">${year-i}</option></c:when>
						...
						<select name="entrDt_mon" id="entrDt_mon" class="form-control input-sm" style="width:80px;float:left;">
							<c:forEach var="i" begin="1" step="1" end="12">
								...
								<c:if test="${gubun eq 'E' }">
									<c:choose>
										<c:when test="${fn:substring(tchSchsVO.entrDt,5,7) eq i }"><option selected value="${ i }">${ i }</option></c:when>
										<c:otherwise><option value="${ i }">${ i }</option></c:otherwise>
									</c:choose>
								</c:if>
							</c:forEach>
				...
				<td class="top">
					<div class="input-group">
						<select name="gradDt_year" id="gradDt_year" class="form-control input-sm" style="width:100px;float:left;">
							<c:forEach var="i" begin="0" end="${year-1900}">
								<c:choose>
									<c:when test="${fn:substring(tchSchsVO.gradDt,0,4) eq (year-i)}"><option selected value="${year-i}">${year-i}</option></c:when>
									<c:otherwise><option value="${year-i}">${year-i}</option></c:otherwise>
								</c:choose>
							</c:forEach>
						</select>
						<select name="gradDt_mon" id="gradDt_mon" class="form-control input-sm" style="width:80px;float:left;">
							<c:forEach var="i" begin="1" step="1" end="12">
								...
								<c:if test="${gubun eq 'E' }">
									<c:choose>
										<c:when test="${fn:substring(tchSchsVO.gradDt,5,7) eq i }"><option selected value="${ i }">${ i }</option></c:when>
										<c:otherwise><option value="${ i }">${ i }</option></c:otherwise>
									</c:choose>
								</c:if>
							</c:forEach>
			...
			<tr height="35">
				...
				<td class="top">
					<input type="text" maxlength="50" lenCheck="50" name="schlNm" value="${vo.schlNm }" id="schlNm" class="form-control input-sm" isNull="N" dispName="<spring:message code="teacher.title.teacherinfo.school.schlnm"/>" style="width:200px;" />
				</td>
				...
				<td class="top">
					<input type="text" maxlength="50" lenCheck="50" name="schlSubj" value="${vo.schlSubj }" id="schlSubj" class="form-control input-sm" isNull="N" dispName="<spring:message code="teacher.title.teacherinfo.school.schlsubj"/>" style="width:200px;" />
				</td>
			</tr>
			<tr height="35">
				...
				<td class="top">
					<input type="text" maxlength="50" lenCheck="50" name="locatNm" value="${vo.locatNm }" id="locatNm" class="form-control input-sm" isNull="N" dispName="<spring:message code="teacher.title.teacherinfo.school.locatnm"/>" style="width:200px;" />
				</td>
				<th class="top" scope="row"><span style="color:red;">* </span><spring:message code="teacher.title.teacherinfo.school.graddiv"/></th>
				<td class="top">
					<label style="font-weight: normal;"><input type="radio" name="gradDiv" id="gradDivY" value="Y" style="border:0" <c:if test="${tchSchsVO.gradDiv eq 'Y' }"> checked</c:if> /> <spring:message code="teacher.title.teacherinfo.school.graddiv_y"/></label>
					<label style="font-weight: normal; margin-left:10px;"><input type="radio" name="gradDiv" id="gradDivN" value="N" style="border:0" <c:if test="${tchSchsVO.gradDiv eq 'N' }"> checked</c:if> /> <spring:message code="teacher.title.teacherinfo.school.graddiv_n"/></label>
				</td>
			</tr>
		</table>
		...
		<div class="text-right">
			...
			<a href="javascript:delSchool()" class="btn btn-primary btn-sm"><spring:message code="button.delete"/> </a>
			</c:if>
			<a href="/mng/tch/info/editPop?userNo=${tchSchsVO.userNo}" class="btn btn-default btn-sm"><spring:message code="button.close"/> </a>
		</div>
...
<script type="text/javascript">
	...
	function process(cmd) {
		$('#tchSchsForm').attr("action", "/mng/tch/schs/" + cmd);
	...
	function processCallback(resultDTO) {
		...
		if(resultDTO.result >= 0) {
			// 정상 처리
			document.location = "/mng/tch/info/editPop?userNo=${tchSchsVO.userNo}";
		} else {
			// 비정상 처리
		}
	}
