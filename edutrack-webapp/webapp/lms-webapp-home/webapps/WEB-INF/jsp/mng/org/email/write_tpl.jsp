...
<input type="hidden" name="tplType" value="${vo.tplType}"/>
	<table summary="data" class="table table-bordered wordbreak">
		...
		<tr>
			<th scope="row"><label for="tplCts"><spring:message code="common.title.cnts"/></label></th>
			<td colspan="3" class="text-center">
				<textarea name="tplCts" id="tplCts" title="<spring:message code="common.title.cnts"/>" style="height:300px;" class="form-control input-sm">${vo.tplCts}</textarea>
			</td>
		</tr>
		<tr>
			<td colspan="4">
  					...
  					<ul class="dropdown-menu" role="menu"  style="max-height:300px;overflow:auto;">
						<li style="padding:7px;">
							<table class="table table-bordered wordbreak">
								...
								<c:otherwise>
									<c:choose>
									<c:when test="${vo.tplType eq 'SMS' || vo.tplCd eq 'EM006' || vo.tplCd eq 'EM007' || vo.tplCd eq 'EM008'}">
										<tr>
...
<script type="text/javascript">
	...
	function process(cmd) {
		if(!validate(document.emailTplForm)) return;
		if(isEmpty($("#tplCts").val())) {
