	...
	<div style="width:100%">
		<div style="width:50%;float:left;padding:5px;">
			<table summary="<spring:message code="system.title.menu.manage" />" class="table table-bordered">
				...
				<tr>
					...
					<td>
						<div class="radio">
							<label><input type="radio" name="useYn" value="Y" <c:if test="${vo.useYn eq 'Y' or empty vo.useYn }">checked</c:if>/> <spring:message code="common.title.useyn_y"/></label>
							<label class="ml10"><input type="radio" name="useYn" value="N" <c:if test="${vo.useYn eq 'N' }">checked</c:if>/> <spring:message code="common.title.useyn_n"/></label>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class="radio">
							<label><input type="radio" name="sslYn" value="Y" <c:if test="${vo.sslYn eq 'Y'  }">checked</c:if>/> <spring:message code="common.title.useyn_y"/></label>
							<label class="ml10"><input type="radio" name="sslYn" value="N" <c:if test="${vo.sslYn eq 'N' or empty vo.sslYn}">checked</c:if>/> <spring:message code="common.title.useyn_n"/></label>
						</div>
					</td>
				</tr>
				<c:if test="${vo.menuType eq 'HOME' && vo.parMenuLvl eq 0}">
				<tr>
					...
					<td>
						...
						<label style="font-weight: normal; margin-left:10px;">
							<input type="radio" name="topMenuYn" value="N" <c:if test="${vo.topMenuYn eq 'N'  or empty vo.topMenuYn }">checked</c:if>/> <spring:message code="system.title.menu.topuseyn_n"/>
						</label>
					</td>
				</tr>
				</c:if>
				<c:if test="${vo.menuType eq 'HOME' && not empty vo.menuUrl}">
				<tr>
					<th scope="row"><spring:message code="system.title.menu.footer.useyn"/></th>
					<td>
						<label style="font-weight: normal;"><input type="radio" style="border:0" name="divLineUseYn" value="Y" <c:if test="${vo.divLineUseYn eq 'Y' }">checked="checked"</c:if>/><spring:message code="common.title.useyn_y"/></label>
						<label style="font-weight: normal; margin-left:10px;"><input type="radio" style="border:0" name="divLineUseYn" value="N" <c:if test="${vo.divLineUseYn eq 'N' or empty vo.divLineUseYn }">checked="checked"</c:if> /><spring:message code="common.title.useyn_n"/></label>
					</td>
				</tr>
				</c:if>
				<tr>
					<th scope="row"><spring:message code="system.title.menu.viewyn"/></th>
					<td>
						<div class="radio">
							<label><input type="radio" name="menuViewYn" value="Y" <c:if test="${vo.menuViewYn eq 'Y' or empty vo.menuViewYn }">checked</c:if>/> <spring:message code="common.title.viewyn_y"/></label>
							<label class="ml10"><input type="radio" name="menuViewYn" value="N" <c:if test="${vo.menuViewYn eq 'N' }">checked</c:if>/> <spring:message code="common.title.viewyn_n"/></label>
						</div>
					</td>
				</tr>
			</table>
		...
		<div style="width:50%;float:left;padding:5px;">
			<ul class="nav nav-tabs" id="tabMenu">
  				...
  				<li><a href="javascript:onclickTab(2)"><spring:message code="system.title.menu.tab.know"/></a></li>
  				<li><a href="javascript:onclickTab(3)"><spring:message code="system.title.menu.tab.write"/></a></li>
			</ul>
...
<script type="text/javascript">
	...
	function pageList(page) {
		...
		if(page == undefined) page = 1;
		$("#prgmListArea").load(
			cUrl("/mng/org/menu/listPage"),
			{ "pageIndex":page},
			function () {$("#loadingBar").hide();}
		);
	}

	function selectKnow(page) {
		$("#loadingBar").show();
		if(page == undefined) page = 1;
		$("#prgmListArea").load(
			cUrl("/mng/org/menu/selectKnow"),
			{ "orgCd":"${vo.orgCd}", "knowCtgrs" : "${vo.knowCtgrs}", "knowTypes" : "${vo.knowTypes}"},
			function () {$("#loadingBar").hide();}
		);
	}
	...
	function directWrite() {
		...
		if(page == undefined) page = 1;
		$("#prgmListArea").load(
			cUrl("/mng/org/menu/addFormDirect"),
			function () {$("#loadingBar").hide();}
		);
	}
	...
	function selectBbs(bbsNm, bbsCd) {
		$("#menuNm").val(bbsNm);
		$("#menuUrl").val("/home/brd/bbs/listAtclMain?bbsCd="+bbsCd);
		$("#optnCtgrCd1").val("BBS");
	...
	function selectPage(pageNm, pageCd) {
		$("#menuNm").val(pageNm);
		$("#menuUrl").val("/home/org/page/viewPageMain?pageCd="+pageCd);
	...
	function inputDirectUrl() {
		$("#menuUrl").val($("#directUrl").val());
	}
</script>
