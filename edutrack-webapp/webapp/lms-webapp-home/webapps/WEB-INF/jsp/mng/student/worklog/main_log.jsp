...
<c:set var="stdEduRsltWorkLogVO" value="${stdEduRsltWorkLogVO}"/>
...
<div style="width:100%">
	<div class="input-group" style="float:left">
		<select name="regDivCd" id="regDivCd" class="form-control input-sm">
			...
			<option value="${item.codeCd}" <c:if test="${item.codeCd eq stdEduRsltWorkLogVO.regDivCd}">selected="selected"</c:if>>${codeName}</option>
			</c:forEach>
		</select>
	</div>
	<div class="input-group" style="float:left;width:200px;">
		<input type="text" name="searchValue" id="searchValue" class="_enterBind form-control input-sm" placeholder="<spring:message code="user.title.userinfo.name"/>" value="${stdEduRsltWorkLogVO.searchValue}"/>
...
<script type="text/javascript">
	...
	function listPageing(page) {
		...
		var url = generateUrl("/mng/std/eduRsltWorkLog/main", {"stdEduRsltWorkLogVO.crsCreCd":"${stdEduRsltWorkLogVO.crsCreCd}", "curPage":page, "stdEduRsltWorkLogVO.regDivCd": regDivCd, "stdEduRsltWorkLogVO.searchValue":searchValue});
