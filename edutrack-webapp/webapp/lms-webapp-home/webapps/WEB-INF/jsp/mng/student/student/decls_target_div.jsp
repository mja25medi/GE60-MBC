...
<c:set var="studentList" value="${studentList}"/>
	...
	<table summary="<spring:message code="student.title.student.list"/>" class="table table-bordered wordbreak">
		...
		<thead>
			<tr>
				<th scope="col" ><input type="checkbox" name="chkAllTarget" id="chkAllTarget" value="N" style="border:0" title="select all" onclick="checkAllTarget()"/></th>
		...
		<tbody>
			...
			<tr>
				<td><input type='checkbox' name='selTarget' id='selTarget' value='${item.stdNo}' style='border:0' title='common.title.select'></td>
				<td>${status.count}</td>
