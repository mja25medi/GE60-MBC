...
<c:set var="crsReshAnsrVO" value="${vo}" />
<c:set var="crsReshAnsrList" value="${crsReshAnsrList}" />


	<div class="well" style="word-break:break-all;word-wrap: break-word;">
		${reshBankItemVO.reshItemCts}
	</div>
	<table class="table table-bordered wordbreak">
		<thead>
			<tr>
				<c:if test="${crsReshAnsrVO.reshAnsrTypeCd eq 'K'}">
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach items="${crsReshAnsrList}" var="item" varStatus="status">
					<tr>
						<c:if test="${crsReshAnsrVO.reshAnsrTypeCd eq 'K'}">
						<td class="text-right">${crsReshAnsrVO.reshAnsr}</td>
						</c:if>
...
<script type="text/javascript">
	...
	function resultResh(){
		location.href="/mng/course/creCrs/resh/rsltPop"
	...
	function listOpinion(page) {
		location.href="/mng/course/creCrs/resh/mainOpinion"
				+"?crsCreCd=${crsReshAnsrVO.crsCreCd}"
