		...
		<div class="col-lg-12">
			<div class="panel panel-default">
				...
				<div id="userList" style="overflow-y:auto;height:320px;">
					<table summary="<spring:message code="course.title.teacher.manage"/>" style="margin-bottom:0px;" class="table table-bordered wordbreak">
						<colgroup>
...
<script type="text/javascript">
	...
	function listUser() {
		...
		$('#userList').load(cUrl("/mng/course/teacher/listSearchTeacher"), { "crsCd":"${crsTchVO.crsCd}", "userNm":userNm});
	}

	function viewUser(userNo) {
		$('#userView').load(cUrl("/mng/course/teacher/viewSearchTeacher"), { "crsCd":"${crsTchVO.crsCd}", "userNo":userNo});
	}
	...
	function addTeacher(userNo) {
		$.getJSON( cUrl( "/mng/course/teacher/addTeacher"),
				   { "crsCd" : "${crsCd}","userNo" : userNo },			// params
				   function(data) {
						if(data.result >= 0) {
							//-- 정상 처리
							alert("<spring:message code="course.message.teacher.add.success"/>");
							listUser();
							//closeView();
							parent.subWorkFrame.listTeacher();
			 	  		} else {
			 	  			//-- 비정상 처리
			 	  			alert("<spring:message code="course.message.teacher.add.failed"/>");
			 	  		}
					}
				);
	}
