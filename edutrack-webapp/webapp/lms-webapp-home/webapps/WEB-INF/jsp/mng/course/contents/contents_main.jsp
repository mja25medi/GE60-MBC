							...
							<div id="contentsWriteArea" style="border:2px solid #4FADBC;width:100%;height:310px;margin-top:7px;">
								<div id="contentsWriteForm" style="width:100%;display:none;">
									...
									<table style="width:96%;margin-top:5px;margin-left:10px;font-size:14px;">
										...
										<tr id="zoomUrlTr" style="display: none;">
											<td style="padding:2px;">
...
<script type="text/javascript">
	...
	function contentsList() {
		var sbjCd = '${vo.sbjCd}';
		$('#contentsList').load(
				cUrl("/mng/course/contents/listContentsJson"),
				{
					"sbjCd" : sbjCd
				},
				function () { }
	...
	function cntsEditForm(unitCd) {
		$("#contentsWriteArea").show();
		$('#contentsList').css('height', '398px');
		var sbjCd = '${vo.sbjCd}';
		$.getJSON( cUrl("/mng/course/contents/editFormContents"),	{
				...
				$("#contentsForm").find('input[name="mobileFilePath"]').val(item.mobileFilePath);
				$("#contentsForm").find('input[name="atchFilePath"]').val(item.atchFilePath);
				$("#contentsForm").find('input[name="bookmarkCnt"]').val(item.bookmarkCnt);
				$("#contentsForm").find('input[name="critTm"]').val(item.critTm);
				$("#contentsForm").find('input[name="zoomUrl"]').val(item.zoomUrl);
				...
				$("#contentsForm").find('input[id="cntsTypeCd1"]').prop("disabled", false);
				...
				if(item.cntsTypeCd == 'CDN') {
					$("#contentsForm").find('input[id="cntsTypeCd1"]').prop("checked", true);
					
					$("#metaTr").css("display", "none");
			   		$("#zoomUrlTr").css("display", "none");
					
				}else if(item.cntsTypeCd == 'VOD') {
					$("#contentsForm").find('input[id="cntsTypeCd2"]').prop("checked", true);
					
					$("#metaTr").css("display", "none");
			   		$("#zoomUrlTr").css("display", "none");
					
				}else if(item.cntsTypeCd == 'MLINK') {
					$("#contentsForm").find('input[id="cntsTypeCd3"]').prop("checked", true);
					
					$("#metaTr").css("display", "none");
			   		$("#zoomUrlTr").css("display", "none");
					
				}else if(item.cntsTypeCd == 'FILE') {
					$("#contentsForm").find('input[id="cntsTypeCd4"]').prop("checked", true);
					
					$("#metaTr").css("display", "none");
			   		$("#zoomUrlTr").css("display", "none");
					
				}else if(item.cntsTypeCd == 'CODING_L') {
					$("#contentsForm").find('input[id="cntsTypeCd5"]').prop("checked", true);
					
					$("#metaTr").css("display", "none");
			   		$("#zoomUrlTr").css("display", "");
					
				}else if(item.cntsTypeCd == 'CODING_T') {
					$("#contentsForm").find('input[id="cntsTypeCd6"]').prop("checked", true);
					
					$("#metaTr").css("display", "none");
			   		$("#zoomUrlTr").css("display", "");
					
				}else if(item.cntsTypeCd == 'META') {
					$("#contentsForm").find('input[id="cntsTypeCd7"]').prop("checked", true);
					
					$("#metaTr").css("display", "");
			   		$("#zoomUrlTr").css("display", "none");
			   		
			   		getMetaScenes(item.sceneId);
				}
	...
	function cntsSort() {
		...
		var addContent = "<iframe id='addCategoryFrame' name='addCategoryFrame' width='100%' height='100%' "
			+ "frameborder='0' scrolling='no' src='<c:url value="/mng/course/contents/sortFormCntsPop"/>"
			+ "?sbjCd="+sbjCd+"'/>";
		modalBox.clear();
