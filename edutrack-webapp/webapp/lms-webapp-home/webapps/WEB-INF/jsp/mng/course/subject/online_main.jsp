...
<script type="text/javascript">
	...
	function listSubject(page) {
		...
		$('#subjectList')
			.load(
				cUrl("/mng/course/subject/listOnline"),
				{ 
				  "sbjNm":sbjNm,
	...
	function sbjWrite() {
		var addContent  = "<iframe id='addFrame' name='addFrame' width='100%' height='100%' "
			+ "frameborder='0' src='<c:url value="/mng/course/subject/addFormOnlinePop"/>"
			+ "?sbjCtgrCd="+ItemDTO.sbjCtgrCd+"'/>";
		modalBox.clear();
	...
	function sbjEdit(sbjCd) {
		var addContent = "<iframe id='addFrame' name='addFrame' width='100%' height='100%' "
			+ "frameborder='0' src='<c:url value="/mng/course/subject/editFormOnlinePop"/>"
			+ "?sbjCd="+sbjCd+"'/>";
		modalBox.clear();
		modalBox.addContents(addContent);
	 ...
	 * 교재 관리 폼
	 */
	function contentsMgr(sbjCd) {
		document.location.href=cUrl("/mng/course/contents/contentsMain")+"?sbjCd="+sbjCd;
	}

	/**
	 * 시험 문제은행 관리 폼
	 */
/* 	function examMgr(sbjCd) {
		document.location.href = cUrl("/mng/course/examQbank/examQbankMain")+"?sbjCd="+sbjCd;
	} */

	/**
	 * 과제 문제은행 관리 폼
	 */
	function assignmentMgr(sbjCd) {
		/*
		var addContent  = "<iframe id='assignmentFrame' name='assignmentFrame' width='100%' height='100%' "
						+ "frameborder='0' scrolling='yes' src='<c:url value="/CourseAssignmentQbankManage.do"/>"
						+ "?cmd=mainAssignmentQbank&assignmentQbankCategoryDTO.sbjCd="+sbjCd+"'/>";
		sbjPopBox.clear();
		sbjPopBox.addContents(addContent);
		sbjPopBox.resize(900, 600);
		sbjPopBox.setTitle("과제 문제은행 관리");
		sbjPopBox.show();
		//sbjPopBox.move(10,10);
		*/
		var url = cUrl("/CourseAssignmentQbankManage.do")+"?cmd=mainAssignmentQbank&assignmentQbankCategoryVO.sbjCd="+sbjCd;
		var option = "width=900, height=600, toolbar=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no";
		var asmtWin = window.open(url,'asmtWin', option);
		asmtWin.focus();
	}

	function subCntInfo(sbjCd) {
		var addContent = "<iframe id='addFrame' name='addFrame' width='100%' height='100%' "
			+ "frameborder='0' scrolling='auto' src='<c:url value="/mng/course/subject/viewSubInfoPop"/>"
			+ "?sbjCd="+sbjCd+"'/>";
		modalBox.clear();
		modalBox.addContents(addContent);
