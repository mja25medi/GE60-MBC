...
<c:set var="gubun" value="${gubun}"/>
    ...
    <table summary="<spring:message code="course.title.course.manage"/>" class="table table-bordered wordbreak">
		...
		<tr>
			...
			<td>
				<span id="ctgrNm">${openCrsCtgrVO.ctgrNm }</span>
			</td>
		</tr>
		<tr>
			<th scope="row"><spring:message code="course.open.title.course.name"/></th>
			<td>
				<input type="text" dispName="<spring:message code="course.open.title.course.name"/>" maxlength="200" isNull="N" lenCheck="200" name="crsNm" value="${vo.crsNm }" class="form-control input-sm"/>
			</td>
		</tr>
		<tr>
			<th scope="row"><spring:message code="course.open.title.course.info"/></th>
			<td>
				<textarea style="height:50px" dispName="<spring:message code="course.open.title.course.info"/>" isNull="N" lenCheck="2000" name="crsDesc" class="form-control input-sm" id="ctgrDesc">${vo.crsDesc }</textarea>
			</td>
		</tr>
		<tr>
    ...
    <div class="text-right">
		...
		<a href="#none" onclick="addCrs()" class="btn btn-primary btn-sm"><spring:message code="button.write"/></a>
        </c:if>
        <c:if test="${gubun eq 'E'}">
        <a href="#none" onclick="editCrs()" class="btn btn-primary btn-sm"><spring:message code="button.edit"/></a>
        <a href="#none" onclick="delCrs()" class="btn btn-warning btn-sm"><spring:message code="button.delete"/></a>
        </c:if>
