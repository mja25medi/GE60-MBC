...
<c:set var="clibShareMediaCntsVO" value="${vo}"/>
<mhtml:mng_html>
...
<mhtml:frm_body cssTag="background-color:#fff;">
	...
	<table summary='<spring:message code="library.title.contents.share"/>' class="table table-striped table-bordered wordbreak">
		...
		<tr>
			...
			<td>
				${clibShareMediaCntsVO.cntsNm }
			</td>
		</tr>
		<tr>
			<th scope="row"><spring:message code="library.title.contents.ccl"/></th>
			<td>
				<meditag:orgcodename code="${clibShareMediaCntsVO.cclCd}" category="CCL_CD" orgCd="${USER_ORGCD }"/>
			</td>
		</tr>
		<tr>
			...
			<td>
				<div class="input-group">
					...
					<input type="text" name="ctgrNm" id="ctgrNm" class="form-control input-sm" style="background-color:#ffffff;" readonly="readonly" value="${clibShareMediaCntsVO.ctgrNm }"/>
					<input type="hidden" name="clibShareMediaCntsVO.ctgrCd" id="ctgrCd" value="${clibShareMediaCntsVO.ctgrCd }">
				</div>
			</td>
		</tr>
		<tr>
			...
			<td>
				<select name="clibShareMediaCntsVO.shareReqStsCd" id="shareReqStsCd" class="form-control input-sm">
					<option value="01" <c:if test="${clibShareMediaCntsVO.shareReqStsCd eq '01' }">selected</c:if>><spring:message code="library.title.sharecd.01"/></option>
					<option value="03" <c:if test="${clibShareMediaCntsVO.shareReqStsCd eq '03' }">selected</c:if>><spring:message code="library.title.sharecd.03"/></option>
					<option value="04" <c:if test="${clibShareMediaCntsVO.shareReqStsCd eq '04' }">selected</c:if>><spring:message code="library.title.sharecd.04"/></option>
				</select>
			</td>
		</tr>
	</table>
