...
<table summary="<spring:message code="library.title.contents.olc.page.manage"/>" class="table table-striped table-bordered wordbreak" style="width:100%;">
	...
	<tr>
		...
		<td>
			<input type="text" name="pageNm" dispName="<spring:message code="common.title.title"/>" isNull="N" lenCheck="50" maxlength="50" class="form-control input-sm" id="pageNm" />
		</td>
	</tr>
	<%-- <tr>
		<th scope="row"><spring:message code="library.title.contents.olc.page.type"/></th>
		<td>
			<label><nested:radio name="pageDiv" value="create" class="pageDiv"/> <spring:message code="library.title.contents.olc.page.write.create"/></label>
			<label style="margin-left:20px;"><nested:radio name="pageDiv" value="link" class="pageDiv"/> <spring:message code="library.title.contents.olc.page.write.link"/></label>
		</td>
	</tr> --%>
	<tr id="page-create">
		<th scope="row"><spring:message code="course.title.contents.mediacontents"/></th>
		<td>
			<label><input type="radio" value="contents" name="mediaType" onchange="mediaCtgr()" checked><spring:message code="library.title.contents.olc.page.media.content"/></label>
			<label style="margin-left:20px;"><input type="radio" value="share" name="mediaType" onchange="mediaCtgr()"><spring:message code="library.title.contents.olc.page.media.share"/></label>
			<br/>
			<div class="input-group">
				...
				<input type="text" name="ctgrNmList" id="ctgrNmList" class="form-control input-sm" style="width:30%;background-color:#ffffff;" readonly="readonly" value="${clibOlcCntsVO.ctgrNm}"/>
	...
	<tr id="page-create">
		<td colspan="2" style="padding:0px;">
			<div id="summernote" style="width:100%; margin: 0 auto;">${vo.pageCts}</div>
	...
	<tr id="page-link" style="display:none">
		...
		<td>
			<input type="text" dispName="<spring:message code="library.title.contents.olc.page.file.select"/>" maxlength="100" isNull="Y" name="filePath" class="form-control input-sm" id="filePath" readonly="true" />
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		dirTree = $('#folderDiv').jstree({
			...
			"json_data" : {
				"ajax" : {
					"url" : cUrl("/mng/library/clibOlcFile/listDir"),
					"data" : function (n) {
		...
		.bind("rename.jstree", function(e, data) {
			...
			if(validateFileName(newFileName)) {
	            ...
	            $.post(
					cUrl("/mng/library/clibOlcFile/renameDir"),
					{
						"filePath" : filePath,
						"fileNm" : fileName,
		...
		function createNode(obj) {
			$.ajaxSetup({async: false});
			$.post(
				cUrl("/mng/library/clibOlcFile/createDir"),
				{
		...
		function removeNode(obj) {
			$.ajaxSetup({async: false});
            $.post(
				cUrl("/mng/library/clibOlcFile/removeDir"),
				{
					"filePath" : obj.attr("filePath")+"/"+obj.attr("fileNm")
				},
				function(result) {
	...
	function mediaCtgr(){
		...
		olcCtgrTree = $('#ctgrTreeArea').jstree({
			...
			"json_data" : {
				"ajax" : {
					"url" : cUrl("/mng/library/clibCntsCtgr/" + cmd),
					"data" : function (n) {
						return {
							"id" : n.attr ? n.attr("id") : "#"
						};
	...
	function setSelectNode(data) {
		...
		var url = "";
		if(ctgrType=="share"){
			url = cUrl("/mng/library/clibShareCnts" + cmd);
			cmd = "listMediaCntsForOlc";
			if(ItemDTO.nodeId != 'root' && ItemDTO.nodeId != '') {
				$("#mediaContents").load(
						url, {
								"clibShareMediaCntsVO.ctgrCd":ItemDTO.nodeId,
							}
				);
			}
		} else  {
			url = cUrl("/mng/library/clibMediaCnts" + cmd);
			cmd = "listOlc";
			if(ItemDTO.nodeId != 'root' && ItemDTO.nodeId != '') {
				$("#mediaContents").load(
						url, {
	 ...
	 */
	function listFile() {
		var url = cUrl("/mng/library/clibOlcFile/listFileCnts")
	...
	function processCallback(resultDTO) {
		if(!isNull(resultDTO.returnMessage)){
	...
	function delPage() {
		if(confirm('<spring:message code="olc.message.contents.confirm.delete"/>')) {
			$('#clibOlcCntsForm').attr("action", "/mng/library/clibOlcCnts/deletePage");
			$('#clibOlcCntsForm').ajaxSubmit(processCallback);
		} else {
			return;
		}
	...
	function setSummernote(){
		var width = $("#summernote").width();
		if(width > 800) width = 800;
		var height = width * 0.6;
		var mediaSrc = $("#mediaContents").val()
		var editorHtml = $('.note-editable').html();
		var videohtml = "<iframe frameborder='0' src='"+mediaSrc+"' width='"+width+"' height='"+height+"' class='note-video-clip'></iframe>";
