		...
		<div class="box">
			<div class="box-body">
						<div class="well well-sm">
							<form name="statsConnForm" id="statsConnForm" onsubmit="return false">
...
<script type="text/javascript">
	...
	function listStatus() {
		//if(!validate(document.statsConnForm)) return;
	    var f = document.statsConnForm;
		$('#statsConnForm').attr("action", "/mng/stats/conn/listConn");
		$('#statsConnForm').ajaxSubmit(processListStatusCallback);
	}
	...
	function processListStatusCallback(resultListVO) {
		...
		for (var i=0; i<list.length; i++) {
			var item = list[i];
			var ratio = Math.round((parseInt(item.connCnt,10) / totCnt) * 100);
			if(i > 0) pieData += ",";
...
// 		                    formatter: "%p.0%, %s",
	...
	function dateSetter(str) {
		if(!validate(document.statsConnForm)) return;
		$('#statsConnForm').attr("action", "/mng/stats/conn/viewAutoDate");
		$('#statsConnForm').find('input[name=dateType]').val(str);
		$('#statsConnForm').ajaxSubmit(autoDateCallback);
	}
	
	function autoDateCallback(ProcessResultDTO) {
		//var Item = ProcessResultDTO.returnDto;
		var Item = ProcessResultDTO;
		$('#statsConnForm').find('input[name=startDt]').val(Item.startDt);
		$('#statsConnForm').find('input[name=endDt]').val(Item.endDt);
		listStatus();
	}
	...
	function excelDownload() {
		...
		$("#statsConnForm").removeAttr('onsubmit');
		$("#statsConnForm").attr('target', '_m_download_iframe');
		$('#statsConnForm').attr("action", "/mng/stats/conn/connListExcelDownload");
		$("#statsConnForm").submit();
		$("#statsConnForm").removeAttr('target');
		$("#statsConnForm").attr('onsubmit', 'return false');
