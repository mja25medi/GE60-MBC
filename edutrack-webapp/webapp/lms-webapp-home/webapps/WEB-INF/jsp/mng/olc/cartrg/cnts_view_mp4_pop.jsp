...
<div data-key="${flowplayerKey}"
	...
	<c:if test="${USER_DEVICE eq 'PC'}">
		...
		<source type="video/flash" src="mp4:${cntsBase}/${olcCntsVO.userNo}/${olcCntsVO.cntsFilePath}"><%-- 와우자 서버를 사용하는경우(vod_smart_yn=Y) 기본 적으로 flash engine을 통해 rtmp 스트리밍을 지원하도록 설정 해 준다.--%>
			<c:if test="${osMac eq 'Mac' }">
		<source type="video/mp4" src="${mediaStreamUrl}${cntsBase}/${olcCntsVO.userNo}/${olcCntsVO.cntsFilePath}/${mediaStreamHls}"> <!-- MAC OS 만 통과-->
			</c:if>
			<c:if test="${osMac ne 'Mac' }">
		<source type="video/mp4" src="${cntsBase}/${olcCntsVO.userNo}/${olcCntsVO.cntsFilePath}">   <!-- Mac 빼고 나머지는 http 전송 -->
			</c:if>
		</c:if>
		<c:if test="${mediaStreamUse ne 'use'}">
		<source type="video/mp4" src="${cntsBase}/${olcCntsVO.userNo}/${olcCntsVO.cntsFilePath}">
		</c:if>
	</c:if>
	<c:if test="${USER_DEVICE ne 'PC'}"> <!-- 모바일 또는 디바이스 확인 안되는 경우  -->
		<c:if test="${mediaStreamUse eq 'use' && USER_OS eq 'ios'}">
		<source type="video/mp4" src="${mediaStreamUrl}${cntsBase}/${olcCntsVO.userNo}/${olcCntsVO.cntsFilePath}/${mediaStreamHls}"> <!-- ios , android 4.3 통과 -->
		</c:if>
		<c:if test="${mediaStreamUse ne 'use' ||  USER_OS ne 'ios'}">
		<source type="video/mp4" src="${cntsBase}/${olcCntsVO.userNo}/${olcCntsVO.cntsFilePath}"> <%-- android가 스트리밍 시 단말기 및 브라우저에 따라 오류 발생하는 양상이 다르게 나오므로  wowzq+mp4+ios 조건  이외는 모두 http 전송 --%>
		</c:if>
	</c:if>
	</video>
</div>
