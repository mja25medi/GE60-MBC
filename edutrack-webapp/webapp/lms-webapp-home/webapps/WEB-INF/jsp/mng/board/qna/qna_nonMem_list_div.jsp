					...
					<tbody id="tbodyList">
						...
						<tr>
							...
							<td class='left wordbreak'>
								<a href="javascript:viewQstn('${item.qnaSn}','${vo.curPage}')">${item.qsCts}</a>
							</td>
							...
							<td class="text-center">
								<c:choose>
									<c:when test="${item.ansYn eq 'Y'}">답변완료</c:when>
							...
							<td class="text-center">
								<c:choose>
									<c:when test="${item.ansYn eq 'Y'}">
										<a class="btn btn-default btn-sm" href="#none" onclick="javascript: editAnsr(${item.qnaSn})" title="${item.qsCts}">답변수정</a>
									</c:when>
								    <c:otherwise>
								    	<a class="btn btn-primary btn-sm" href="#none" onclick="javascript: writeAnsr(${item.qnaSn})" title="${item.qsCts}">답변작성</a>
								    </c:otherwise>
								</c:choose>
							</td>
						</tr>
