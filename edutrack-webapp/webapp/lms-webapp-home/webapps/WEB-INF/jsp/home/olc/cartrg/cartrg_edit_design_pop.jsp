...
<body style="padding-top: 0px;">
	...
	<table summary='<spring:message code="course.title.category.manage"/>' class="table table-striped table-bordered">
		...
		<tr>
			...
			<td >
				<input type="text" maxlength="20" style="width:100px;" dispName="<spring:message code="olc.title.olcname.color"/>" name="olcNameColor" isNull="Y" lenCheck="20" class="form-control input-sm" id="olcNameColor"  />
			</td>
			<th scope="row"><label for="listBgColor"><spring:message code="olc.title.cartridge.list.bgcolor"/></label></th>
			<td>
				<input type="text" maxlength="20" style="width:100px;" dispName="<spring:message code="olc.title.cartridge.list.bgcolor"/>" name="listBgColor" isNull="Y" lenCheck="20" class="form-control input-sm" id="listBgColor"/>
			</td>
		</tr>
		<tr>
			<th scope="row"><label for="listFontColor"><spring:message code="olc.title.cartridge.list.fontcolor"/></label></th>
			<td>
				<input type="text" maxlength="20" style="width:100px;" dispName="<spring:message code="olc.title.cartridge.list.fontcolor"/>" name="listFontColor" isNull="Y" lenCheck="20" class="form-control input-sm" id="listFontColor"/>
			</td>
			<th scope="row"><label for="contentBgColor"><spring:message code="olc.title.cartridge.content.bgcolor"/></label></th>
			<td>
				<input type="text" maxlength="20" style="width:100px;" dispName="<spring:message code="olc.title.cartridge.content.bgcolor"/>" name="contentBgColor" isNull="Y" lenCheck="20" class="form-control input-sm" id="contentBgColor"/>
			</td>
		</tr>
		<tr>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		topLogoFile = new $M.JqueryFileUpload({
							"varName"			: "topLogoFile",
							"files" 			: $.parseJSON('${vo.topLogoImgJson}'),
		...
		<c:if test="${vo.bkgImgSn ne '' || vo.bkgImgSn ne '0'}">
		$("#bkgimgArea").html(
				'<div id="">'
				+'<a href="javascript:fileDown(\'${vo.bkgImgSn }\');" style="cursor:pointer;" title="Download: image">'
				+'<img src="/app/file/thumb/${vo.bkgImgSn }">'
				+'</a>'
				+'<a class="btnRemoveFile" id="btnRemoveFile_${vo.bkgImgSn }" onclick="delBkgImg();" href="#_none">'
		...
		<c:if test="${vo.olcNameColor ne null && vo.olcNameColor ne '' }">
		$('#olcNameColor').css("background-color","#${vo.olcNameColor}");
		</c:if>
		<c:if test="${vo.listBgColor ne null && vo.listBgColor ne '' }">
		$('#listBgColor').css("background-color","#${vo.listBgColor}");
		</c:if>
		<c:if test="${vo.contentBgColor ne null && vo.contentBgColor ne '' }">
		$('#contentBgColor').css("background-color","#${vo.contentBgColor}");
		</c:if>
		<c:if test="${vo.listFontColor ne null && vo.listFontColor ne '' }">
		$('#listFontColor').css("background-color","#${vo.listFontColor}");
		</c:if>
	});
	...
	function listBkgImg() {
		$('#cartrgDiv').hide();
		$('#bkgImgfDiv').load(
			"/home/olc/cartrg/editCartrgDesignBkgImgForm"
		);
		$('#bkgImgfDiv').show();
	}
