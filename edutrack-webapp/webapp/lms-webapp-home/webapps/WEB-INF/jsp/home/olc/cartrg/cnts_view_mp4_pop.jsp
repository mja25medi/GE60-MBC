...
<div data-key="${flowplayerKey}"
	...
	<c:if test="${USER_DEVICE eq 'PC'}">
		...
		<source type="video/mp4" src="${wowzaUrlHttp}${cntsBase}/${vo.userNo}/${vo.cntsFilePath}/playlist.m3u8"> <!-- MAC OS 만 통과-->
			</c:if>
	...
	<c:if test="${USER_DEVICE ne 'PC'}"> <!-- 모바일 또는 디바이스 확인 안되는 경우  -->
		...
		<source type="video/mp4" src="${wowzaUrlHttp}${cntsBase}/${vo.userNo}/${vo.cntsCts}/playlist.m3u8"> <!-- ios , android 4.3 통과 -->
		</c:if>
