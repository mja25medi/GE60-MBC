...
<div class="userView">
	...
	<c:choose>
		...
		<div class=contentViewList style="float:left;width:260px;margin-left:840px; ${listColor}">
			...
			<c:forEach var="item" items="${olcPageList}" varStatus="status">
...
<script type="text/javascript">
	...
	function viewPage(page) {
		...
		var pagePerRatio = Math.round(1 / ${fn:length(olcPageList)} * 100,0);
		var gapRatio = 100 - (pagePerRatio * ${fn:length(olcPageList)});
		...
		var url = cUrl("/lec/bookmark/viewOlcPageMain")+"?cntsCd=${clibOlcCntsVO.cntsCd}&amp;pageCd="+pageInfo;
		var addContent  = "<iframe id='viewCntsFrame' name='viewCntsFrame' width='100%' height='660px' "
			+ "frameborder='0' scrolling='yes' src='"+url+"'/>";
		$("#workArea").html(addContent);
		$("#pageArea").html(ItemDTO.current+" / ${fn:length(olcPageList)}");
