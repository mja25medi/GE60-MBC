...
<c:set var="itemList" value="${reshItemList}"/>
<c:url var="img_base" value="/img/home" />
				
                    <div class="table_list">
                        <ul class="list">
                            <li class="head"><label>설문명</label></li>
                            <li>${orgReshVO.reshTitle}</li>
                        </ul>
                        <ul class="list">
                            <li class="head"><label>설문기간</label></li>
                            <li><meditag:dateformat type="8" delimeter="." property="${orgReshVO.startDttm}"/> ~<meditag:dateformat type="8" delimeter="." property="${orgReshVO.endDttm}"/></li>
                        </ul>
                         <ul class="list">
			                <li class="head"><label>설문내용</label></li>
			                <li>${orgReshVO.reshCts}</li>
			            </ul>
                    </div>
                    
							<form id="orgReshForm" name="orgReshForm" onsubmit="return false">
							<input type="hidden" name="reshSn" value="${orgReshAnsrVO.reshSn}" />
							<input type="hidden" name="reshAnsrStr" value="${orgReshAnsrVO.reshAnsrStr}"/>
                            <!-- 게시판 -->
                            ...
                            <div class="survey_area mt30">
                                    ...
                                    <div class="survey_list">
                                       <div class="title">${status.count}. ${fn:replace(fn:replace(item.reshItemCts,"<p>",""),"</p>","")}<small >
                                       				<c:if test="${item.reshItemTypeCd eq 'D'}">(서술형/최대 1000자)</c:if>
                                        		   <c:if test="${item.reshItemTypeCd eq 'K'}">(선택형)</c:if></small>
                                       </div>
										...
										<ol>
										 ...
										 <c:if test="${item.reshItemTypeCd eq 'D'}">
                                        	<textarea name="etcOption" id="etcOption" class="form-control etc${item.reshItemSn }" placeholder="서술형 답 입력영역" rows="5" maxlength="1000"></textarea>
                                        </c:if>
...
<script type="text/javascript">
	...
	var reshSn = '${orgReshAnsrVO.reshSn}';
	var userNo = '${orgReshAnsrVO.userNo}';
	...
	function process(cmd) {
		$('#orgReshForm').attr("action","/home/resh/"+cmd);
		document.orgReshForm.submit();
	}

	function goList() {
		document.location.href = cUrl("/home/resh/main");
	}
	...
	function submitVV(){
		...
		$("#orgReshForm").attr("action","/home/resh/join");
	...
	function processCallback(resultDTO) {
		alert(resultDTO.message);
			...
			if(resultDTO.result >= 0) {
				location.href="/home/resh/main";
				parent.modalBoxClose();
			}
			else{
			location.href="/org/resh/joinMain?"+"${orgReshAnsrVO.crsTermCd}"+"&"+"${orgReshAnsrVO.reshSn}";
			}
	}
    ...
    function loadAnsr() {
    	$.ajax({
    		...
    		success : function(data) {
    			if(data.length > 0) {
    				for(vo of data) {
    	    			if(vo.reshAnsrTypeCd == 'K') {
    	    				$("input[name=expl" + vo.reshItemSn + "][value=" + vo.reshAnsr + "]").attr("checked", true);
    	    			}
    	    			else if(vo.reshAnsrTypeCd == 'D') {
    	    				$(".etc" + vo.reshItemSn).val(vo.reshAnsr);
    	    			}
    				}
    			}
    		}
