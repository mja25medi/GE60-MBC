...
<c:set var="clibOlcPageVO" value="${vo}"/>
							...
							<div class="btn-group divPage" role="group" id="${item.pageCd }">
								...
								<button type="button" class="btn btn-default btn-sm ct_txt listPage <c:if test="${vo.pageCd eq item.pageCd }">active</c:if>" onclick="viewPage('${item.pageCd }')" title="${item.pageNm}" id="list_${item.pageCd }">
