	...
	<input type="hidden" id="crsCreCd" name="crsCreCd" value="${vo.crsCreCd}">
				...
				<c:if test="${bbsInfoVO.cmntUseYn eq 'Y'}">
					...
					<c:if test = "${vo.regNo eq sessionScope.USERNO }">
						...
						<button type="button" onclick="deleteAtcl();" class="btn type2">삭제</button>
					</c:if>
                     <c:if test="${bbsInfoVO.writeUseYn eq 'Y'}">
	                     <c:if test="${bbsInfoVO.ansrUseYn eq 'Y' && not empty sessionScope.USERNO}">
	                    	 ...
	                    	 <button type="button" onclick="addFormAtcl();" class="btn">답변</button>
	                     </c:if>
                     </c:if>
                    </div>
...
<script type="text/javascript">
		...
		function addFormAtcl(){
			...
			var url = generateUrl("/home/brd/bbs/addFormAtclMain", 
				{	
