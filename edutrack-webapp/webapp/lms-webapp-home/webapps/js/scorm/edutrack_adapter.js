...
function EdutrackAPIAdapter() {
	...
	this.SetConnPageCnt					= ApiConnPageCnt;
...
function ApiInitialize(parameter) {
	...
	if (parameter != null && parameter != "") {
		...
		if (this.isInitialized == true) {
			...
			jQuery.getJSON( cUrl("/lec/bookmark/indexViewBookmark"),		// url
					{ 
...
function ApiInitializeCallback(resultDTO) {
	var outBookmarkDTO = resultDTO.returnVO;
	...
	if (result == "true") {
		...
		this.BookmarkDTO.connCnt		= parseInt(outBookmarkDTO.connCnt,10) + 1;
...
function ApiCommit(parameter) {
	...
	else if(parameter == undefined || parameter == "" || parameter == "fin") {
		if (this.isInitialized == true) {
			// credit 인 경우만 저장
			if (this.credit == "credit") {
				this.endTime = getTimeStamp('end');

				this.BookmarkDTO.connTm = ((this.endTime - this.startTime ) * 0.6);
				...
				if(this.bookmarkType == "PAGE") {
					var connPageCnt = this.connPageCnt;
					var totalPageCnt = this.totalPageCnt;
					this.BookmarkDTO.connPageCnt = connPageCnt;
					this.BookmarkDTO.totPageCnt = totalPageCnt;
					var ratio = connPageCnt / totalPageCnt;
					...
					if(this.showTime == "" || this.showTime == "0" || parameter == "fin") {
						this.BookmarkDTO.prgrRatio = 100;
					} else {
//						var ratio = parseInt(this.BookmarkDTO.connTotTm,10) / (parseInt(this.showTime,10)*60);
						var ratio = parseInt(this.BookmarkDTO.seekTime,10) / (parseInt(this.showTime,10)*60);
//						var ratio = this.progressRatio;
						console.log("ratio",ratio);
						console.log("seekTime:",this.BookmarkDTO.seekTime);
						
						if(parseInt(this.BookmarkDTO.prgrRatio) != 0 && parseInt(ratio*100) <= parseInt(this.BookmarkDTO.prgrRatio)){//기존 진도율보다 작으면 기존 진도율
							ratio = this.BookmarkDTO.prgrRatio/100;
						}
				...
				jQuery.getJSON( cUrl("/lec/bookmark/indexAddBookmark"),		// url
						{ 	
...
function ApiSetTotalPageCnt(totalPageCnt) {
	...
	this.BookmarkDTO.totalPageCnt = totalPageCnt;
}

function ApiConnPageCnt(connPageCnt) {
...
function getTimeStamp(str) {
	var ts = 0;
	jQuery.getJSON( cUrl("/lec/bookmark/indexGetTimeStamp"),		// url
			{ 	
				"param" : str
			},			// params
