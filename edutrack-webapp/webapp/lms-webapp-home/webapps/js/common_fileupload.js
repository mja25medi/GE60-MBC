...
$M.JqueryFileUpload = function(option) {
	...
	this.data = {
		...
		//"returnValueId"		: "",	// 업로드 완료시 파일 아이디를 넣어 줄 필드의 id 값
		"onUploadFunction"	: "",	// 업로드 완료시 호출하는 펑션
		"onRemoveFunction"	: "",	// 삭제 완료시 호출하는 펑션
...
$M.JqueryFileUpload.prototype = {
		...
		add 	:
			...
			function(e, data) {
				...
				$('#'+this.data.progressId).hide();
				if(this.data.onUploadFunction) {
					eval(this.data.onUploadFunction+"()");
				}
			},
		fail	:
			function(e, data) {
		...
		removeCallBack	:	// 파일 삭제 콜백 (실패하면 메시지만 표시)
			function(response) {
				...
				this.data.onAppend();
				if(this.data.onRemoveFunction) {
					eval(this.data.onRemoveFunction+"()");
				}
			},
