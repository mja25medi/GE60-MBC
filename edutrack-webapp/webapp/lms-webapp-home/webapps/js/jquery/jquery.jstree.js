	...
	$(function() {
		...
		var u = navigator.userAgent.toLowerCase(),
			...
			css_string = '' +
				...
				'.jstree a { display:inline-block; line-height:16px; height:18px; color:black; white-space:nowrap; text-decoration:none; padding:1px 2px; margin:0; } ' +
				'.jstree a:focus { outline: none; } ' +
				'.jstree a > ins { height:18px; width:16px; } ' +
...
(function ($) {
	$.vakata.context = {
		...
		show	: function (s, t, x, y, d, p) {
			...
			if(y + h > ($(document).height()-10)) { //-- $(document) -> $(".jstree")로 변경
				y = y - (h + t[0].offsetHeight);
				if(y < 0) y = 0; //-- 상단으로 넘어가는 메뉴 막기 위해
		...
		parse	: function (s, is_callback) {
			...
			$.each(s, function (i, val) {
				...
				if(val.icon && val.icon.indexOf("/") !== -1) { str += " style='background:url(" + val.icon + ") center 3px no-repeat;' "; }
	...
	$(function () {
		var css_string = '' +
			'#vakata-contextmenu { display:none; position:absolute; margin:0; padding:0; min-width:140px; background:#ebebeb; border:1px solid silver; z-index:10000; *width:140px; } ' +
			'#vakata-contextmenu ul { min-width:140px; *width:140px; } ' +
			'#vakata-contextmenu ul, #vakata-contextmenu li { margin:0; padding:0; list-style-type:none; display:block; } ' +
			'#vakata-contextmenu li { line-height:22px; min-height:22px; position:relative; padding:0px; } ' +
			'#vakata-contextmenu li a { padding:1px 6px; line-height:19px; display:block; text-decoration:none; margin:1px 1px 0 1px; } ' +
			'#vakata-contextmenu li ins { float:left; width:18px; height:18px; text-decoration:none; margin-right:6px; margin-left:2px;} ' +
